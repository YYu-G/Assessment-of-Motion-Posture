...
def report_shape(ownerID,photo,model_id):
    ...
    shape_image(model_path,tp,save_path,txt_path)
    file_name={}
    fp={}
...
def report_fitness(ownerID,video,model_id):
    ...
    exercise_counter(pose_model='../yolov8s-pose.pt',  # pose模型
                    detector_model_path=model_file_path,  # 训练完的检测姿态模型路径
                    detector_model_file=model_name,#模型名称
                    video_file=tp,  # 视频文件
...
def report_yoga(owner,video):
    ...
    timestamp = datetime.utcnow().isoformat()
    file_name = f"{timestamp}_{video.filename}"
    video.save(os.path.join(rep_file_path, file_name))
    # 获取当前的日期和时间
    now = datetime.now()
    # 只获取日期部分
    current_date = now.date()
